...
# ======================================================
# Vistas de secciones nuevas - PESCADERÍA
# - Ofertas, Productos Destacados, Promociones
# ======================================================
def ofertas(request):
    """Vista para mostrar productos en oferta (con descuento > 0)"""
    productos_oferta = Producto.objects.filter(descuento__gt=0).order_by('-descuento')
    categorias = Categoria.objects.all()
...
def nosotros(request):
    """Página institucional con información del local y su ubicación"""
    return render(request, 'productos/nosotros.html')


def promociones(request):
    """Vista para mostrar promociones especiales"""
    # Ejemplo: productos con descuento mayor al 20%
    productos_promocion = Producto.objects.filter(descuento__gte=20).order_by('-descuento')
    categorias = Categoria.objects.all()
    return render(request, 'productos/promociones.html', {
        'productos': productos_promocion,
        'categorias': categorias
    })
